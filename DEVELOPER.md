...
and contributing to the MySQL Gemini CLI Extension project.
...
2.  **MySQL Instance:** For testing data plane tools, you will need access to an active MySQL instance and database.
...
1.  **Clone the Repository:**
    ...
    ```bash
    git clone https://github.com/gemini-cli-extensions/mysql.git
    cd mysql
    ```
...
4.  **Testing Changes:** After installation, start the Gemini CLI (`gemini`).
    You can now interact with the `mysql` tools to manually test your changes
...
*   `@gemini-cli-extensions/mysql-maintainers`
